...
// pagination
if( !function_exists('lebotheme_pagination') ){

    function lebotheme_pagination( $current_query = null ){
...
// comments
if( !function_exists('lebotheme_comment') ){

    function lebotheme_comment($comment, $args, $depth) {
        ...
        echo '<span>';
		echo esc_html__('at', 'lebotheme') . ' ' . get_comment_time() . ', ' . get_comment_date() . ' - ';
		comment_reply_link(array_merge( $args, array('reply_text' => esc_html__('Reply', 'lebotheme'), 'before' => '', 'after' => '', 'add_below' => $add_below, 'depth' => $depth, 'max_depth' => $args['max_depth'])));
		echo '</span>';
		echo '</p>';
		
        echo '<div class="comment-text">';
        if ($comment->comment_approved == '0'){
            echo '<em>' . esc_html__("Your comment is awaiting moderation", 'lebotheme') . '</em><br />';
        }
        comment_text();
...
if( !function_exists('lebotheme_commentform_title') ){
	function lebotheme_commentform_title( $args ) {
...
add_filter( 'comment_form_defaults', 'lebotheme_commentform_title' );

// the caption displayed within single blog post hero pages
if( !function_exists('lebotheme_blog_post_hero_caption') ){

    function lebotheme_blog_post_hero_caption() {
