jQuery(function ($) {
	...
	$(document).ready(function() {
		...
		console.log('---- WELCOME TO LEBOSTUDIO ----');
...
function MenuPicture(){
	...
	items.each(function(){
		...
		var picture  = item.find('.menu-picture');

		if(picture.length) {
			itemLink.on("mouseenter", function(){
				//var item = jQuery(this);
				item.addClass('actifLink');
				picture.addClass('actif');
				TweenLite.to(picture, .5, {
					css: {
						opacity:1,
			...
			itemLink.on("mouseleave", function(){
				...
				picture.removeClass('actif')
				TweenLite.to(picture, 0.5, {
					css: {
						opacity:0,
						transform:"rotate(0)"
					}
...
---------------------------------------------------*/	
	...
	function FirstLoad() {	
		...
		$('#burger-wrapper, .menu .button-text').on('click', function() {
			...
			$('body').toggleClass('smooth-scroll')
			setTimeout( function(){			
...
---------------------------------------------------*/	
	function SplittingTitles(){
		...
		if( w >= 1000 ){
			...
			//var tweenFirst = TweenMax.from(spanTop, 0.7, {transform: "translateX(30px)"})
			// var tweenSecond = TweenMax.from(spanBottom, 0.7, {transform: "translateX(-40px)"})
			...
			timeline.add(TweenMax.from(spanTop, 0.7, {transform: "translateX(30px)"})).add(TweenMax.from(spanBottom, 0.7, {transform: "translateX(30px)"}));
			scene.setTween(timeline)
			scene.addTo(controller)
			//scene.addIndicators({name: "titre (duration: 300)"}) // add indicators (requires plugin)
			...
			var postImageScene = new ScrollMagic.Scene({triggerElement: ".post-image", offset:-200, duration: 600})
			//postImageScene.addIndicators({name: "thumbnail (duration: 300)"}) // add indicators (requires plugin)
